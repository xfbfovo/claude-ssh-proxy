...
func main() {
	dbPath := flag.String("db", "sshproxy.db", "SQLite 数据库文件路径")
	...
	log.Printf("ssh-proxy %s 启动中...", version)
	...
	if n, _ := store.CountAdminUsers(); n == 0 {
		pw := randomPassword()
		if err := store.CreateAdminUser(*adminUser, pw); err != nil {
			log.Fatalf("创建初始管理员账号失败: %v", err)
		}
		log.Printf("========================================")
		log.Printf("已创建初始管理员账号,请立即登录并修改密码:")
