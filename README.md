...
- proxy 侧的认证支持:多个公钥 + 一个可选密码,任一种匹配即可登录
- proxy 连目标机器时的认证支持:密码或私钥,存在数据库里,由管理员在 Web 后台维护
- 所有配置(路由、管理员账号、审计日志)存在 SQLite 单文件数据库里,改配置即时生效,不需要重启 SSH 监听(除非改的是监听地址本身)
...
- SSH 监听 `:2222`
- Web 管理后台监听 `:8080`
- 数据库文件 `sshproxy.db`(当前目录)

首次启动会自动创建一个管理员账号,密码会打印在启动日志里,例如:

```
========================================
已创建初始管理员账号,请立即登录并修改密码:
  用户名: admin
  密码:   b17eefc237249eeb0b9c2b91
========================================
```

请立即打开 `http://<部署机器>:8080` 用这个账号登录,并在"监听设置"页面修改密码。
...
- **允许连接此别名的客户端公钥**:可以填多个,给 Agent 侧的私钥对应的公钥
- **登录密码(可选)**:如果不想用公钥,也可以给这个别名单独设一个密码,两种方式都能登录 proxy
...
- `.github/workflows/release.yml`:推送 `vX.Y.Z` 格式的 tag(例如 `v0.0.1`)会自动触发,交叉编译 Linux amd64/arm64 两个版本的二进制,打包成 `.tar.gz` 并发布到 GitHub Release
